...
COMPANIES_VC = ['Лукойл', 'Роснефть', 'Сбербанк', 'Новатэк', 'Сургутнефтегаз','Северсталь', 'Норникель',
                'Транснефть', 'Газпром', 'Росатом']

# Список имен, которые мы будем присваивать БД

COMPANIES_DB = ['lukoil', 'rosneft', 'sberbank', 'novatek', 'surgutneftegas', 'severstal', 'nornickel',
                'transneft', 'gazprom', 'rosatom']
...
def main():
    ...
    for x in range(len(COMPANIES_VC)):
        ...
        # Читаем вакнсии из json и подготавливаем данные для записи в БД
